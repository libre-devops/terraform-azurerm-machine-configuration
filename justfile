...
e2e stack="minimal":
    ...
    # from LDO_CONFTEST_POLICIES when set, otherwise shallow-clones the public terraform-rego-policies repo.
    function Invoke-LdoConftestPlan {
        ...
        try {
            if (-not ($pol -and (Test-Path $pol))) {
                ...
                git clone --depth 1 --branch main https://github.com/libre-devops/terraform-rego-policies.git $cfClone 2>&1 | Out-Null
    ...
    $path = 'examples/{{ stack }}'
    $key = 'terraform-azurerm-machine-configuration-{{ stack }}.tfstate'
    $added = $false
    try {
